...
VALUES (
    'riskDecisionFlow',
    '通用风控决策流程',
    '基于 LiteFlow 的标准风控决策流程，包含基础检测、规则执行、AI分析和决策判定',
    'PUBLISHED',
    1
) ON CONFLICT (code) DO NOTHING;

-- ============================================================
-- 2. 动态决策阈值初始化（全局默认，不区分流程/事件/用户等级）
-- ============================================================
...
-- 3. 登录类规则初始化
-- ============================================================
INSERT INTO rule_config (code, name, type, expression, score, enabled, priority)
VALUES
    ('login_new_device',    '新设备登录',   'login', 'isNewDevice == true',    20, TRUE, 10),
    ('login_abnormal_time', '异常时间登录', 'login', 'loginHour < 6',          15, TRUE, 20),
    ('login_frequent_fail', '频繁登录失败', 'login', 'failedLoginCount >= 3',  30, TRUE, 30)
ON CONFLICT (code) DO NOTHING;
...
INSERT INTO rule_config (code, name, type, expression, score, enabled, priority)
VALUES
    ('payment_large_amount',  '大额支付',     'payment', 'amount > 10000',          40, TRUE, 10),
    ('payment_medium_amount', '中额支付',     'payment', 'amount > 5000',           20, TRUE, 20),
    ('payment_first_large',   '首次大额充值', 'payment', 'isFirstRecharge == true', 15, TRUE, 30)
ON CONFLICT (code) DO NOTHING;
...
INSERT INTO rule_config (code, name, type, expression, score, enabled, priority)
VALUES
    ('register_with_invite', '邀请码注册（降低风险）', 'register', 'inviteCodeUsed == true', -10, TRUE, 10)
ON CONFLICT (code) DO NOTHING;
