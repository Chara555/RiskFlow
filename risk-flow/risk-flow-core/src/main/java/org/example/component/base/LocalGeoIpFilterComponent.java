...
public class LocalGeoIpFilterComponent extends AbstractRiskComponent {
    ...
    private static final String RULE_NAME = "GEO_IP_CHECK";
    ...
    protected RiskSignal doEvaluate(RiskFlowContext context) {
        ...
        if (HIGH_RISK_REGIONS.contains(regionCode)) {
            ...
            return RiskSignal.hit(
                    ...
                    String.format("GeoIP 解析为高危地区: %s", regionCode),
                    java.util.Map.of("regionCode", regionCode, "ip", ip),
        ...
        return RiskSignal.pass(
                RULE_NAME,
                String.format("GeoIP resolved to standard region: %s", regionCode),
                String.format("GeoIP 解析为正常地区: %s", regionCode)
        );
    }
}
